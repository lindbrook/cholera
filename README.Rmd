...
```{r latlong_voronoi, fig.align = "default", fig.width = 5.5, fig.height = 5.5, fig.show = "hold", out.width = "50%", echo = FALSE}
plot(neighborhoodVoronoi(), euclidean.path = TRUE)
...
nearest.pump <- do.call(rbind, lapply(cases$anchor, function(x) {
  ...
  d <- vapply(pumps$id, function(p) {
    m2 <- as.matrix(pumps[pumps$id == p, vars])
...
invisible(lapply(cholera::voronoi.polygons, function(x) {
  polygon(x[, vars])
}))
```
