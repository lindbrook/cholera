...
plot.walking_path <- function(x, zoom = TRUE, add = FALSE, long.title = TRUE,
  ...
  orig <- unique(data.summary$origin)
    ...
    if (type == "cases") {
      if (all(dest < 1000L)) {
        ...
        text(fatality[fatality$case == dest, vars], pos = 1, labels = dest,
          col = "red")
      } else if (all(dest >= 1000L & dest < 2000L)) {
  ...
  if (!add) longTitle(long.title, type, pmp, orig, dest, land, x, path.data)
}
...
caseCase <- function(origin, destination, case.set, network, orgn.dstn,
  ...
  if (length(ego.node) == 1) {
    ...
    d.multi.ego <- lapply(ego.node, function(x) {
      if (weighted) {
        igraph::distances(graph = g, v = x, to = alter.node, weights = edges$d)
      } else {
        igraph::distances(graph = g, v = x, to = alter.node)
      }
    })
