#' Compute orthogonal rectangle for range of map data (prototype).
#'
#' To be georeferenced for use as polygon filter.
#' @param pad Numeric. Padding adjustment percent [0, 1].
#' @export

orthogonalRectanlge <- function(pad = 0.01) {
  rng <- mapRange()
  frm <- stats::setNames(expand.grid(rng$x, rng$y), c("x", "y"))
  frm <- frm[c(1, 2, 4, 3), ]
  delta <- mean(c((rng$x[2] - rng$x[1]) * pad, (rng$y[2] - rng$y[1]) * pad))
  delta <- round(delta, 2)
  for (i in seq_along(frm$x)) {
    if (i == 1) {
      frm[i, ] <- frm[i, ] - delta
    } else if (i == 2) {
      frm[i, ] <- frm[i, ] + c(delta, -delta)
    } else if (i == 3) {
      frm[i, ] <- frm[i, ] + delta
    } else if (i == 4) {
      frm[i, ] <- frm[i, ] + c(-delta, delta)
...
#' PDF orthogonal rectangle  (prototype).
#'
#' @param path Character. e.g., "~/Documents/Data/"
#' @export

orthogonalRectanlgePDF <- function(path) {
  rng <- mapRange()
  grDevices::pdf(file = paste0(path, "ortho.rect.pdf"))
  plot(orthogonalRectanlge(), pch = 46, xlim = rng$x, ylim = rng$y,
    xaxt = "n", yaxt = "n", xlab = NA, ylab = NA, bty = "n", asp = 1)
  grDevices::dev.off()
}

#' Compute latitude and longitude for georeferenced orthogonal rectangle.
#'
#' Uses georeferenced TIF from QGIS.
...
latlongOrthoRect <- function(path, tif, k = 4, multi.core = TRUE) {
  ...
  # arbitrarily set by eye
  sel <- map.data$x > -0.145 & map.data$x < -0.13 &
         map.data$y > 51.508 & map.data$y < 51.518
  ...
  do.call(rbind, coords)[c(1, 2, 4, 3), ]
}

# ortho.rect <- latlongOrthoRect(path, tif)
# usethis::use_data(ortho.rect, overwrite = TRUE)
