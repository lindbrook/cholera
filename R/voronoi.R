...
#' }
#' @seealso \code{\link{addVoronoi}}, \code{vignette("pump.neighborhoods")}
#' @export
#' @examples
#' neighborhoodVoronoi()
...
neighborhoodVoronoi <- function(pump.select = NULL, vestry = FALSE,
  ...
  if (is.null(pump.select)) {
    coordinates <- deldirPolygons(pump.data, cholera::roads)
  } else {
    coordinates <- deldirPolygons(pump.data[pump.select,], cholera::roads)
  }
