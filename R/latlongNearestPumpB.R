...
latlongNearestPumpB <- function(pump.select = NULL, metric = "walking",
  ...
  if (metric == "euclidean") {
    ...
    dat <- latlongNeighborhoodDataB(vestry)
    path.data <- latlong_pathDataB(dat, pump.select, weighted, vestry,
      cores)
...
latlong_pathDataB <- function(dat, pump.select, weighted, vestry, cores) {
  g <- dat$g
  edge.list <- dat$edge.list
  edges <- dat$edges
  ortho.addr <- latlongOrthoAddressB()
  ortho.pump <- latlongOrthoPumpB(vestry = vestry)
