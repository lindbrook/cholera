#' Embed anchors, landmarks and/or pumps into road network (prototype 2).
#'
...
embedNodesB <- function(vestry = FALSE, case.set = "observed",
  ...
  if ((isTRUE(embed.addr) | is.numeric(embed.addr)) &
    ...
    ortho.addr <- orthoAddr(case.set = case.set, latlong = latlong)
    ortho.land <- orthoLand(latlong = latlong)
    ortho.pump <- orthoPumps(vestry = vestry, latlong = latlong,
      drop.isolates = drop.isolates)

    if (is.numeric(embed.addr)) {
      ortho.addr <- ortho.addr[ortho.addr$case %in% embed.addr, ]
    }
  ...
  } else if ((isTRUE(embed.addr) | is.numeric(embed.addr)) &
...
# test <- cholera:::embedNodesB(embed.pump = 7)
# test <- cholera:::embedNodesB(embed.addr = TRUE, embed.pump = TRUE, embed.landmarks = TRUE)
# plot(test$g, vertex.label = NA, vertex.size = 2)

orthoAddr <- function(case.set = "observed", latlong = FALSE) {
  if (latlong) {
...
orthoPumps <- function(vestry = TRUE, latlong = FALSE, drop.isolates = FALSE) {
  if (latlong) {
    if (vestry) {
