#' Compute latitude and longitude for orthogonal case projection (anchor case).
#'
...
latlongOrthoAnchor <- function(multi.core = FALSE, radius = 60) {
  cores <- multiCore(multi.core)

  geo.anch <- geoCartesian(dat = cholera::fatalities.anchor, anchor.case = TRUE)
  ...
  anchors.in.fix <- vapply(road.segment.fix, function(x) {
    any(x %in% cholera::fatalities.anchor$anchor)
  }, logical(1L))
  ...
  manual.compute <- lapply(seq_along(road.segment.fix), function(i) {
    ...
    x <- unlist(tmp)
    data.frame(anch = x[x %in% cholera::fatalities.anchor$anchor],
      seg = names(tmp), row.names = NULL)
  })

  manual.compute <- do.call(rbind, manual.compute)
  
  classfication.err <- geo.anch$id %in% manual.compute$anch
  no.err <- geo.anch$id[!classfication.err]

  orthogonal.projection <- parallel::mclapply(no.err, function(id) {
    case <- geo.anch[geo.anch$id == id, c("x", "y")]
  ...
  orthogonal.projectionB <- lapply(seq_len(nrow(manual.compute)), function(i) {
    anch <- manual.compute[i, "anch"]
    seg <- manual.compute[i, "seg"]

    case <- geo.anch[geo.anch$id == anch, c("x", "y")]
  ...
  coordsB <- data.frame(do.call(rbind, orthogonal.projectionB),
    case = manual.compute$anch, row.names = NULL)
  ...
  sel <- names(cholera::road.segments) %in% c("x1", "y1", "x2", "y2")
  geo.seg <- cholera::road.segments[, !sel]
...
# latlong.ortho.anchor <- cholera:::latlongOrthoAnchor(multi.core = TRUE)
# usethis::use_data(latlong.ortho.anchor)
# usethis::use_data(latlong.ortho.anchor, overwrite = TRUE)
