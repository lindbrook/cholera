...
addWalking <- function(pump.select = NULL, vestry = FALSE, weighted = TRUE,
  ...
  if (area) {
    if (x$case.set == "expected") {
      ...
      points(cholera::regular.cases[sel, ],
        col = grDevices::adjustcolor(sim.proj$color, 1/3), pch = 15,
        cex = 1.25)
    }
  } else {
