#' Plots observed shortest path walking neighborhoods (beta).
#'
#' Plots walking neighborhoods based on the weighted shortest paths between observed fatalities and their nearest pump.
#'
...
observedNeighborhood <- function(selection = NULL, vestry = FALSE,
  ...
  if (weighted) {
    path.data <- lapply(wtd.paths, numericNodeCoordinates)
  } else {
    path.data <- lapply(paths, numericNodeCoordinates)
  }
