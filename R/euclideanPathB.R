...
euclideanPathB <- function(origin = 1, destination = NULL, type = "case-pump",
  ...
  if (type == "case-pump") {
    ...
    if (observed) {
      ...
      if (is.null(destination)) {
        alters.id <- seq_len(nrow(cholera::regular.cases))
        ...
        if (is.numeric(destination)) {
          ...
          if (all(destination > 0)) {
            ...
            alters.sel <- seq_len(nrow(cholera::regular.cases)) %in% destination

          } else if (all(destination < 0)) {
            # alters.id <- cholera::sim.ortho.proj[cholera::sim.ortho.proj$case
            #   %in% abs(destination) == FALSE, "case"]

            alters.sel <- seq_len(nrow(cholera::regular.cases)) %in%
              destination == FALSE
          }

          # alters <- cholera::sim.ortho.proj[cholera::sim.ortho.proj$case %in%
          #   alters.id, ]
          alters <- cholera::regular.cases[alters.sel, ]
          alters.id <- alters.id[alters.sel]
          ...
          if (grepl("Square", destination)) {
            ...
            alters.id <- cholera::landmarks[cholera::landmarks$name == destination,
              "case"]
    ...
    if (identical(all.equal(ego.id, alters.id), TRUE)) {
      ...
      if (observed) {
        ...
        vars <- c("x", "y")
        alters <- alters[seq_len(nrow(alters)) != ego.id, ]
...
print.euclidean_pathB <- function(x, ...) {
...
plot.euclidean_pathB <- function(x, zoom = TRUE, radius = 0.5,
  ...
  if (is.null(unit.posts)) {
    # arrows(ego.xy$x.proj, ego.xy$y.proj,
    #        alter.xy$x.proj, alter.xy$y.proj,
    #        col = case.color, lwd = 3, length = 0.075)

    arrows(ego.xy$x, ego.xy$y, alter.xy$x, alter.xy$y,
           col = case.color, lwd = 3, length = 0.075)
    ...
    if (unit.posts %in% c("distance", "time") == FALSE) {
      ...
      # invisible(lapply(a.lst), function(i) {
