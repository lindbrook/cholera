...
#' @param unit.posts Character. "distance" for mileposts; "time" for timeposts. NULL for none.
...
addWalkingPathB <- function(origin = 1, destination = NULL, type = "case-pump",
  ...
  time.unit = "second", walking.speed = 5, unit.posts = NULL,
  unit.interval = NULL, alpha.level = 0.67, path) {
  ...
  if (x$type == "case-pump") {
    ...
    if (alpha.level != 1) {
      case.color <- grDevices::adjustcolor(dist.col, alpha.f = alpha.level)
    } else {
      case.color <- dist.col
    }
