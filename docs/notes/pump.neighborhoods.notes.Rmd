...
For both types, I use "expected" data to compute the expected neighborhood area. Using `sp::spsample()` and `sp::`polygon()``, I place 20K regularly-spaced points across the face of the map (in the "real world", points are approximately 6 meters apart.).^[Because the map frame is not rectangular, the actual number of points is 19,993.]
...
```{r traveling, fig.align = "left", fig.width = 5, fig.height = 5, echo = TRUE, eval = TRUE}
neighborhood <- neighborhoodWalking(-6, case.set = "expected", vestry = TRUE)
