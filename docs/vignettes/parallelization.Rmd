...
The 'cholera' package supports parallelization via the 'parallel' R package. It is _off_ by default. To enable it, either set `multi.core = TRUE`, which uses all logical cores, or pass the number of cores the `multi.core` argument (e.g., `multi.core = 2`). To check the number of physical/logical cores on your system, use `parallel::detectCores()`. 
...
Besides courtesy, parallelization is now off by default because coding changes have improved performance for intensive tasks like computing and plotting area polygons for "expected" cases in neighborhoodWalking().

## Benchmarks

The two tables below compare the timings (in seconds) for 'cholera' versions 0.8.0 and 0.9.0 for the following expressions:
...
The timings below reflect the average of 5 evaluations of the above expressions using the 'microbenchmark' package, R version 4.4.3 and macOS 13.7.4 on a 3.1 GHz Dual-Core Intel Core i5 processor.
