...
By default, for reasons discussed below, parallelization is turned off. For functions that support it, to enable it you need only set "multi.core = TRUE". You can also pass a value for the number of logical cores to use. To avoid paging to disk, you should having adequate RAM. A conservative estimate is that a thread/job can take up to 500 MB. That means if you're running on 8 cores, eight threads will take approximately 4 GB of memory.
...
The timings below (in seconds) including those for parallel:parLapply(), which are used to support parallelization on Window, were done on a 2.3 Ghz Intel Core i7 using the 'microbenchmark' package with R version 3.6.1 on macOS 10.14.6.
