...
In 1992, Rusty Dodson and Waldo Tobler digitized the map. Each bar and water pump was given a unique x-y coordinate. Each road was translated into a set of straight line segments, defined by their endpoints. While the original data are no longer available,^[http://www.ncgia.ucsb.edu/pubs/snow/snow.html] they are preserved in Michael Friendly's _HistData_ R package. The data are plotted below:
...
Despite their appeal, I would argue that these stacks are visually and computationally problematic. The reason is that not all bars are created equal. Even though they are identical in terms of their appearance and the only thing that seems to distinguish them is their position on the map, bars actually play two different roles. Sometimes a bar represent the location of a fatality, sometimes it doesn't. Standalone bars (i.e., a stack with a one bar or an addresses with one fatality), or the bar at the base of a stack do. Bars above the base case do not: their only purpose is to create the visual stacking effect and represent the number of fatalities at that address.

This duality can be problematic for a map. Because a map is a visual device that illustrates spatial relationships, it's natural to assume that the positions of all elements (e.g., bars) reflect actual physical locations. So when we violate this principle, we undermine the visual interpretation of the map and handicap our analysis. This is particularly true when 44% (257/578) of bars in Snow's map fall into this geographically uninformative category.

To address these problems, I "unstack" Dodson and Tobler's data. I do so in two different data sets. The first, _fatalities.unstacked_, "unstacks" the data so that all cases in a stack (at the same "address") are given the same x-y coordinate. The second, _unstacked.address_, "unstacks" the data by changing the unit of observation from the case to the stack (i.e., "address"). Each address is a single observation with a single x-y coordinate, and the number of cases at that location becomes an attribute of the address.
