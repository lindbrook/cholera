...
What makes it so is that Snow adds a graphical annotation of the "neighborhood", or set of residences, that he believes is most likely to make use of the Broad Street pump:
...
To allow users to explore the data with this approach, I wrote neighborhoodVoronoi(). It creates an R object that describes one's desired neighborhood configuration. The results can be visualized with plot() and summarized with summary(). 

The figure below plots the "addresses" and Voronoi cells for the 13 pumps in the original map.

```{r, fig.width = 5.25, fig.height = 5.25, fig.align = "center", message = FALSE}
plot(neighborhoodVoronoi())
```

The dataframe below summarizes the results in the map. Pearson is (Count - Expected) / sqrt(Expected):

```{r}
summary(neighborhoodVoronoi())
```

The next figure excludes the Carnaby and Little Marlborough Street pump (#6).

```{r, fig.width = 5.25, fig.height = 5.25, fig.align = "center", message = FALSE}
plot(neighborhoodVoronoi(-6))
```

The dataframe with summary results for this configuration lie below:

```{r}
summary(neighborhoodVoronoi(-6))
```

The share of addresses that fall within the Broad Street Voronoi pump neighborhood increases from 57% to 66%.
...
The plot below excludes the pump at Carnaby and Little Marlborough Street (#6) and recomputes the paths.
...
```{r}
summary(neighborhoodWalking(-6))
```

The share of addresses that fall within the Broad Street pump walking neighborhood increases from 59% to 70%.
