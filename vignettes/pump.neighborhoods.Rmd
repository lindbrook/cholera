...
The plot below excludes the pump, mentioned and illustrated above in the Voronoi neighborhood section, at Carnaby and Little Marlborough Street (#6).
...
And by identifying neighborhoods by coloring regions.^[Mackenzie (N.D) uses this approach.]
...
neighborhoodWalking() is computationally intensive. With default arguments and a single core, it takes about 1-2 minutes to run on a 2.3 GHz Intel Core i7.^[With 4 physical cores and 8 logical ones, it takes about 45 seconds.] The function performs two signifcant tasks: 1) it computes the shortest walking path between each observed or simulated case and all selected pumps; and 2) to reduce overplotting it computes the minimal set of segments neede to represent all walking paths withiin a given neighborhood.

If you want to explore a number of different neighborhoods, it may be better to pre-compute the data or to run in batch mode. For convenience, three configurations will return pre-computed results: 1) the default set of arguments, which uses all pumps; 2) the default set excluding the pump on Little Marlborough Street (pump 6); and 3) the default set with just the Little Marlborough Street and the Broad Street pumps (6 and 7).
