...
In 1992, Rusty Dodson and Waldo Tobler digitized the map. However, the data are no longer available.^[http://www.ncgia.ucsb.edu/pubs/snow/snow.html] Fortunately, they are preserved in Michael Friendly's _HistData_ R package. The amended data from this package are plotted below:
...
However, Dodson and Tobler did not include the names of roads (e.g., Broad Street) in their efforts. While not strictly necessary to the task of visualizing the spatial distribution of cases, having the names can be of help to analysts. To that end, I append the actual street names to the data. This appended data set is called _roads_.
...
Before discussing road names, some discussion of the structure of _roads_ is warranted. The data contain 1241 x-y coordinates that are used to define the endpoints for 528 "streets", which are defined by one or more line segments.
...
This complexity reflects three thing about the data. First, a "street" does not necessarily correspond to a street in the "real-world". For example, 50 of the 528 "street" segments are used to draw the rectangular border that frames Snow's map. Second, the remaining 478 "street" segments actually describe only 206 "real-world" roads (e.g., Oxford Street, Regent Street). Third, 40% of "real-world" roads are composed of multiple "street" segments. For example, Oxford Street consists of 26 line segments and Broad Street consists of 6.
...
To help locate and visualize streets and road segments (including map frame segments), you can use the _streetLocatorName_() and _streetLocatorNumber_() functions.

To match names in the _roads_ data set, _streetLocatorName_() corrects for case and removes extraneous spaces.
