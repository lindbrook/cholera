...
In 1992, Rusty Dodson and Waldo Tobler digitized the map. While the original data are no longer available,^[http://www.ncgia.ucsb.edu/pubs/snow/snow.html] they are preserved in Michael Friendly's _HistData_ R package. The amended data from that package are plotted below:
...
Unfortunately, Dodson and Tobler did not include the names of roads (e.g., Broad Street) in their effort. While not strictly necessary to the task of visualization, having the names is useful. To that end, I append the actual street names to the data. This appended data set is called _roads_.

While great effort was made to get the names right, there may still be errors. Error reports and suggestions for amendments are welcome.

## Road data

Before discussing road names, some discussion of the structure of _roads_ is warranted. The data contain 1241 x-y coordinates that are used to define the endpoints for 528 "streets", which are defined by one or more straight line segments.
...
This complexity reflects three things about the structure of the road data. First, a "street" does not necessarily correspond to a real-world street. For example, 50 of the 528 "street" segments are used to draw the rectangular frame around Snow's map. Second, the remaining 478 "street" segments actually describe 206 "real-world" roads (e.g., Oxford Street, Regent Street). Third, 40% of "real-world" roads are composed of multiple "street" segments. For example, Oxford Street consists of 26 line segments and Broad Street consists of 6.
...
The primary source for names was Snow's map. A high resolution version is available [here](http://www.ph.ucla.edu/epi/snow/highressnowmap.html).

While great effort was made to read and cross-reference names, there may still be errors. Some roads on the map do not have a name. So I attached unique labels like "Unknown-D". Some names appear multiple times even though they lie at different locations. For these, I use Roman numerals to distinguish them (e.g., "King Street (I)" and "King Street (II)").

## Queen Street (I) and Marlborough Mews

There is one apparent coding error in Dodson and Tobler's road data. Queen Street (I) extends too far: the water pump (#7) that is clearly located on Marlborough Mews (see [map](http://www.ph.ucla.edu/epi/snow/highressnowmap.html), cited above), ends up on Queen Street (I).

I amend this by moving the end point of Queen Street (I) westward so that the street only runs north-south. I do so by reassigning the segment that runs east-west so that it is now part of Marlborough Mews.
...
This amended _roads_ data frame is included in the _cholera_ package.

## Finding streets by name or road segment number

To help locate and visualize streets and road segments (including map frame segments), you can use the _streetNameLocator_() and _streetNumberLocator_() functions. Note that _streetNameLocator() uses the names in the _roads_ data set. However, the fuctions tries to corrects for case and removes extraneous spaces: streetNameLocator("Oxford Street") and streetNameLocator("oxford street")

### Examples

```{r, fig.width = 5.25, fig.height = 5.25, fig.align = "center", echo = TRUE}
streetNameLocator("Oxford Street")
streetNameLocator("Cambridge Street", zoom = TRUE, radius = 0.5)
streetNumberLocator(50)
```
