...
In 1992, Rusty Dodson and Waldo Tobler digitized the map. While the original data are no longer available,^[http://www.ncgia.ucsb.edu/pubs/snow/snow.html] they are preserved in Michael Friendly's _HistData_ R package. The amended version of those data, which this package calls _fatalities_, are plotted below:
...
Unfortunately, Dodson and Tobler did not include the names of roads (e.g., Broad Street) in their effort. While not strictly necessary to for analysis or visualization, having the names is useful. To that end, I append the actual street names, taken from the map itself, to the data. This appended data set is called _roads_.
...
Before discussing road names, some discussion of the structure of _roads_ is warranted. The data contain 1241 pairs of x-y coordinates. These coordinates define the endpoints of the straight line segments used to describe 528 numerically identified "streets". 
...
The primary source for raod names was Snow's map. A high resolution version is available [here](http://www.ph.ucla.edu/epi/snow/highressnowmap.html).

While great effort was made to read and cross-reference names, there may still be some errors. Some roads on the map do not have a name. So I attached unique labels like "Unknown-D". Some names appear multiple times even though they lie at different locations. For these, I use Roman numerals to distinguish them (e.g., "King Street (I)" and "King Street (II)").
...
This amended _roads_ data frame is included in this package.

## Finding roads by name or "street" number

To help locate and visualize streets and road segments (including map frame segments), you can use the _streetNameLocator_() and _streetNumberLocator_() functions. Note that _streetNameLocator() uses the names in the _roads_ data set. However, the function tries to corrects for case and removes extraneous spaces: streetNameLocator("Oxford Street") and streetNameLocator("oxford street"). 
...
## Locating and visualizing individual road segments

For those interested in more granular analysis, there is the _road.segments_ dataframe that uses individual road segments as the unit of observation. 

```{r}
head(road.segments)
```

These segeents can be located and visualized using _segmentLocator()_.

```{r, fig.width = 5.25, fig.height = 5.25, fig.align = "center", echo = TRUE}
segmentLocator("178-1")
```
